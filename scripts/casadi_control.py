...
omega = 5.28
omega2 = omega**2
max_torque = 2 * 0.01223 * 500 # factor of 2 cause I think I messed up the force calculation
opti = casadi.Opti()
...
cost = sum([(cos(x[i]) + 1)**2 for i in range(N)]) + 0.01 * sum([u[i]*u[i] for i in range(N-1)])
