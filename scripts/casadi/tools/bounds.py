...
def reportBounds(
    value, lowerbound, upperbound, labels=None, tol=1e-8, showNonViolating=True
):
    ...
    if not (len(v) == len(lb) and len(lb) == len(ub)):
        raise Exception(
            "value, lowerbound and upperbound must all be the same size, but got %d, %d and %d. "
            % (len(v), len(lb), len(ub))
        )

    if len(labels) != len(v):
        raise Exception(
            "Labels (%d) must be same size as values (%d)" % (len(labels), len(v))
        )
