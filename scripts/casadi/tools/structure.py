...
def canonicalIndexAncestors(ind):
    ...
    return [ind] + canonicalIndexAncestors(
        ind[: -(map(is_string, ind[::-1]).index(True) + 1)]
    )
...
class SafeDict(dict):
    def __getitem__(self, k):
        if k not in self:
            raise Exception(
                "Unknown keyword '%s'. Available entries: %s" % (k, str(self.keys()))
            )
    ...
    def traverseByPowerIndex(
        ...
        try:
            ...
            new_exc = Exception(
                ...
                % (str(powerIndex), str(canonicalIndex), str(e))
            )
    ...
    def __str__(self, compact=False):
        ...
        if compact:
            s += (
                "{"
                + ",".join(
                    k + ": " + v.__str__(compact=True) for k, v in self.dict.items()
                )
                + "}"
            )
        else:
...
class CasadiStructureDerivable:
    ...
    def squared_repeated(self, arg=0):
        ...
        s = struct(
            [entry("t", shapestruct=(self, self), repeat=a.shape[1] / self.size)]
        )
            ...
            if isinstance(a, t):
                numbers = c(
                    ...
                    data=DataReferenceSquaredRepeated(
                        a, self.size, a.shape[1] / self.size
                    ),
                )
            ...
            try:
                ...
                raise CompatibilityException(
                    ...
                    % (
                        str(canonicalIndex),
                        str(self.master),
                        str(__builtin__.type(self.master)),
                        str(payload),
                        str(__builtin__.type(payload)),
                        str(i),
                        str(__builtin__.type(i)),
                        str(e),
                    )
                )
            except Exception as e:
...
class MXVeccatStruct(CasadiStructured, MasterGettable):
    ...
    def master(self):
        if any(e is None for e in self.storage):
            missing = filter(
                lambda k: self.storage[self.mapping[k]] is None, self.mapping.keys()
            )
