...
try:
    ...
    try:
        ...
        raise Exception(
            "To use the functionality of casadi.tools.graph, you need to have pydot Installed. Try `easy_install pydot`."
        )
    ...
    def drawSparsity(self, s, id=None, depid=None, graph=None, nzlabels=None):
        ...
        if s.nnz() == s.numel():
            graph.add_node(
                pydot.Node(
                    id,
                    label="%d x %d" % (s.size1(), s.size2()),
                    shape="rectangle",
                    color=self.sparsitycol,
                    style="filled",
                )
            )
        else:
            ...
            label += (
                "<TR><TD COLSPAN='%d'><font color='#666666'>%s</font></TD></TR>"
                % (s.size2(), s.dim())
            )
            for i in range(s.size1()):
                ...
                for j in range(s.size2()):
                    ...
                    if k == -1:
                        ...
                        label += "<TD PORT='f%d' BGCOLOR='%s'>%s</TD>" % (
                            k,
                            self.sparsitycol,
                            nzlabels[nzlabelcounter],
                        )
                        nzlabelcounter += 1
                label += "</TR>"
            label += "</TABLE>>"
...
class MXSymbolicArtist(DotArtist):
    ...
    def draw(self):
        ...
        if s.nnz() == s.numel() and s.nnz() == 1:
            ...
            # The Matrix grid is represented by a html table with 'ports'
            label = (
                '<<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" COLOR="%s">' % col
            )
            label += (
                "<TR><TD COLSPAN='%d'>%s: <font color='#666666'>%s</font></TD></TR>"
                % (s.size2(), s.name(), s.dim())
            )
            for i in range(s.size1()):
                label += "<TR>"
                for j in range(s.size2()):
                    ...
                    if k == -1:
                        ...
                        label += (
                            "<TD PORT='f%d' BGCOLOR='#eeeeee'> <font color='#666666'>(%d,%d | %d)</font> </TD>"
                            % (k, i, j, k)
                        )
                label += "</TR>"
            label += "</TABLE>>"
...
class MXEvaluationArtist(DotArtist):
    def draw(self):
        ...
        graph = pydot.Cluster(
            str(s.__hash__()), rank="max", label="Function:\n %s" % f.name()
        )
        self.graph.add_subgraph(graph)

        s = (" %d inputs: |" % f.n_in()) + " | ".join(
            "<f%d> %d" % (i, i) for i in range(f.n_in())
        )
        graph.add_node(
            pydot.Node("funinput" + str(self.s.__hash__()), label=s, shape="Mrecord")
        )
    ...
    def draw(self):
        ...
        if len(dep) > 1:
            ...
            for i, n in enumerate(dep):
                graph.add_edge(
                    pydot.Edge(str(n.__hash__()), op + str(k.__hash__()) + ":f%d" % i)
                )
        else:
            s = print_operator(k, ["."])
            self.graph.add_node(
...
class MXSetNonzerosArtist(DotArtist):
    def draw(self):
        ...
        for i in range(s.size1()):
            ...
            for j in range(s.size2()):
                ...
                if k == -1 or Mk >= len(M) or k != M[Mk]:
                    label += "<TD>.</TD>"
                    if Mk < len(M) - 1 and M[Mk] == -1 and k != -1:
                        Mk += 1
                else:
        ...
        self.graph.add_edge(
            pydot.Edge(str(entry.__hash__()), op + str(s.__hash__()) + ":entry")
        )
...
class MXIfTestArtist(DotArtist):
    def draw(self):
        ...
        for i, n in enumerate(dep):
            graph.add_edge(
                pydot.Edge(str(n.__hash__()), str(k.__hash__()) + ":f%d" % i)
            )
...
class MXDensificationArtist(DotArtist):
    def draw(self):
        ...
        self.graph.add_node(
            pydot.Node(str(k.__hash__()), label="densify(.)", shape="oval")
        )
...
class SXLeafArtist(DotArtist):
    def draw(self):
        ...
        self.graph.add_node(
            pydot.Node(
                str(self.s.__hash__()), label=str(self.s), shape="box", style=style
            )
        )
...
def createArtist(node, dep={}, invdep={}, graph=None, artists={}, **kwargs):
    if isinstance(node, SX):
        if node.is_scalar(True):
            if is_leaf(node):
                return SXLeafArtist(
                    node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
                )
            else:
                return SXNonLeafArtist(
                    node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
                )
        else:
            return SXArtist(
        ...
        if node.is_symbolic():
            return MXSymbolicArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_binary() or node.is_unary():
            return MXOperationArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_constant():
            return MXConstantArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_call():
            return MXEvaluationArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_output():
            return MXEvaluationOutputArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_norm():
            return MXNormArtist(
            ...
            return MXGetNonzerosArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_op(C.OP_SETNONZEROS):
            return MXSetNonzerosArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        elif node.is_op(C.OP_ADDNONZEROS):
            return MXAddNonzerosArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
        else:
            return MXGenericArtist(
                node, dep=dep, invdep=invdep, graph=graph, artists=artists, **kwargs
            )
    else:
...
def dotdraw(s, direction="RL", **kwargs):
  ...
  """
    ...
    try:  # Check if we have pylab
        ...
        print(
            "casadi.tools.graph.dotdraw: no pylab detected, will not show drawing on screen."
        )
    ...
    if hasattr(show, "__class__") and show.__class__.__name__ == "PylabShow":
        ...
        figure_name = "%s%d.%s" % (
            show.basename,
            len(show.figure_list),
            show.figure_extension,
        )
