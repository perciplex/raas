...
def canonicalIndexAncestors(ind):
    if len(ind) == 0:
        return []
    return [ind] + canonicalIndexAncestors(
        ind[: -(list(map(is_string, ind[::-1])).index(True) + 1)]
    )
...
class SafeDict(dict):
    def __getitem__(self, k):
        if k not in self:
            raise Exception(
                "Unknown keyword '%s'. Available entries: %s"
                % (k, str(list(self.keys())))
            )
...
def payloadUnpack(payload, i):
    ...
    if isIterable(payload):
        if i >= len(payload):
            raise Exception(
                "Rhs out of range. Got list index %s but rhs list is only of length %s."
                % (i, len(payload))
            )
        return payload[i]
...
class StructEntry:
    ...
    def traverseCanonicalIndex(self, nest=False, limit=1000):
        children = (
            [[]]
            if (self.struct is None or limit == 0)
            else self.struct.traverseCanonicalIndex(limit=limit - 1)
        )
    ...
    def traverseByPowerIndex(
        self, powerIndex, dims=None, canonicalIndex=(), dispatcher=None, payload=None
    ):
        try:
            if dims is None:
                dims = self.dims
            ...
            if len(dims) == 0:
                ...
                if is_integer(p):
                    ...
                    raise Exception(
                        "powerIndex entry {} cannot be used in list context."
                    )
                elif isinstance(p, set):
                    raise Exception(
                        """powerIndex entry {"foo","bar"} cannot be used in list context."""
                    )
                    ...
                    return dispatcher.callableOuter(
                        payload, canonicalIndex, extraIndex=None, entry=None, inner=r
                    )
                else:
                    raise Exception("I don't know what to do with this: %s" % str(p))
        except Exception as e:
            raise Exception(
                "Error occured in entry context with powerIndex %s, at canonicalIndex %s"
                % (str(powerIndex), str(canonicalIndex))
            ) from e
...
class Structure(object):
    def __init__(self, entries, order=None):
        ...
        self.keyslist = sum(
            [list(i) if isinstance(i, tuple) else list([i]) for i in self.order], []
        )
    ...
    def traverseCanonicalIndex(self, limit=1000):
        ...
        for d in self.order:
            if isinstance(d, tuple):
                for v in intersperse(
                    *[
                        self.dict[de].traverseCanonicalIndex(True, limit=limit - 1)
                        for de in d
                    ]
                ):
                    ret += v
    ...
    def getStruct(self, name):
        if name not in self.struct.dict:
            raise Exception(
                "Cannot find entry with key '%s'. Candidates: "
                % (str(name), str(list(name.keys())))
            )
    ...
    def traverseByPowerIndex(
        self, powerIndex, canonicalIndex=(), dispatcher=None, payload=None
    ):
        try:
            if len(powerIndex) == 0:
          ...
          """
            ...
            raise Exception(
                "Error occured in struct context with powerIndex %s, at canonicalIndex %s"
                % (str(powerIndex), str(canonicalIndex))
            ) from e
...
class Dispatcher:
    ...
    def callableOuter(
        self, payload, canonicalIndex, extraIndex=None, entry=None, inner=None
    ):
        return inner
...
class CasadiStructureDerivable:
        ...
        if mtype is None:
            if isinstance(arg, SX):
                ...
                try:
                    ...
                    raise Exception(
                        "Call to Structure has weird argument: expecting DM-like or MX-like or SXMatrix-like"
                    )
    ...
    def squared(self, arg=0):
        ...
        if not (a.shape[1] == a.shape[0] and a.shape[0] == self.size):
            raise Exception(
                "Expecting square DM of size %s. Got %s" % (self.size, a.dim())
            )
    ...
    def product(self, otherstruct, arg=0):
        ...
        if not (a.shape[1] == otherstruct.size and a.shape[0] == self.size):
            raise Exception(
                "Expecting DM of shape (%s,%s). Got %s"
                % (self.size, otherstruct.size, a.dim())
            )
    ...
    def squared_repeated(self, arg=0):
        ...
        if not (a.shape[0] == self.size and a.shape[1] % self.size == 0):
            raise Exception(
                "Expecting square (%d) DM by N. Got %s" % (self.size, a.dim())
            )
        s = struct(
            [entry("t", shapestruct=(self, self), repeat=int(a.shape[1] / self.size))]
        )
        ...
        for (t, c) in [(DM, DMStruct), (MX, MXStruct), (SX, SXStruct)]:
            if isinstance(a, t):
                numbers = c(
                    s,
                    data=DataReferenceSquaredRepeated(
                        a, self.size, int(a.shape[1] / self.size)
                    ),
                )
...
class GetterDispatcher(Dispatcher):
    def __call__(self, payload, canonicalIndex, extraIndex=None, entry=None):
        ...
        if canonicalIndex in self.struct.map:
            ...
            if canonicalIndex in self.priority_object_map and (
                extraIndex is None or len(extraIndex) == 0
            ):
            ...
            i = performExtraIndex(
                self.struct.map[canonicalIndex], extraIndex=extraIndex, entry=entry
            )
            ...
            try:
                ...
                raise Exception(
                    "Error in powerIndex slicing for canonicalIndex %s"
                    % (str(canonicalIndex))
                ) from e
        else:
...
class SetterDispatcher(Dispatcher):
        ...
        if canonicalIndex in self.struct.map:
            ...
            try:
                if type is None:
                    ...
                    iflip = performExtraIndex(
                        self.struct.map[canonicalIndex],
                    ...
                    if payload_.is_scalar():
                        ...
                        oi = performExtraIndex(
                            DM.ones(entry.originalsparsity),
                            extraIndex=extraIndex,
                            entry=entry,
                        )
                ...
                raise CompatibilityException(
                    "Error in canonicalIndex slicing for %s: Incompatible types in a[i]=b with a %s (%s) and b %s (%s) and i %s (%s). Error: %s"
                    % (
                        str(canonicalIndex),
                        str(self.master),
                        str(builtins.type(self.master)),
                        str(payload),
                        str(builtins.type(payload)),
                        str(i),
                        str(builtins.type(i)),
                        str(e),
                    )
    ...
    def callableOuter(
        ...
        try:
            ...
            raise CompatibilityException(
                "Error in canonicalIndex slicing for %s: Incompatible types in a[i]=b with a %s and b %s."
                % (str(canonicalIndex), str(self.master), str(payload))
            )
        except Exception as e:
            raise Exception(
                "Error in powerIndex slicing for canonicalIndex %s"
...
class MasterGettable:
    ...
    def __getitem__(self, powerIndex):
        return self.struct.traverseByPowerIndex(
            powerIndex,
            dispatcher=GetterDispatcher(
                struct=self.struct,
                master=self.master,
                priority_object_map=self.priority_object_map,
            ),
        )
...
class MasterSettable:
    ...
    def __setitem__(self, powerIndex, value):
        return self.struct.traverseByPowerIndex(
            powerIndex,
            dispatcher=SetterDispatcher(
                struct=self.struct, master=self.master, mtype=self.mtype
            ),
            payload=value,
        )
...
def delegation(extraIndex, entry, i):
    if is_string(extraIndex) or (
        isinstance(extraIndex, list)
        and len(extraIndex) > 0
        and all([is_string(e) for e in extraIndex])
    ):
        extraIndex = FlatIndexDelegater(extraIndex)
    if isinstance(extraIndex, Delegater):
        if entry is None:
            raise Exception("Cannot use delayed index without supplied entry.")
        if entry.shapestruct is None:
            raise Exception("Cannot use delayed index without supplied shapestruct.")
        if not (isinstance(entry.shapestruct[i], Structure)):
            raise Exception(
                "Cannot use delayed index with a integer shapestruct argument."
            )
...
def performExtraIndex(i, extraIndex=None, entry=None, flip=False):
    if extraIndex is None or len(extraIndex) == 0:
        return i
    if isinstance(extraIndex[0], collections.Callable) and not isinstance(
        extraIndex[0], Delegater
    ):
        return extraIndex[0](
            performExtraIndex(i, extraIndex=extraIndex[1:], entry=entry, flip=flip)
        )
    if not (isinstance(extraIndex[0], NestedDictLiteral)):
        if len(extraIndex) > 2 or len(extraIndex) == 0:
            raise Exception(
                "Powerindex exhausted. Remaining %s is interpreted as extraIndex, but length must be 1 or 2."
                % str(extraIndex)
            )
        try:
            if len(extraIndex) == 1:
            ...
            raise Exception(
                "Powerindex exhausted. Passing on %s to %s, but it doesn't know what to do with it"
                % (str(extraIndex), str(type(i)))
            )
    else:
        return i
...
class Prefixer:
    ...
    def __str__(self):
        return (
            ...
            + str(self.prefix)
            + ","
            + self.struct.__str__(compact=True)
            + ")"
        )
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in self.traverseCanonicalIndex():
            ...
            self.map[it] = m
            self.lookuptable += [
                (it, kk, p) for kk, p in enumerate(zip(sp.get_col(), sp.row()))
            ]
        ...
        class IMGetter(StructureGetter):
            ...
            def __getitem__(self, powerIndex):
                return self.struct.traverseByPowerIndex(
                    powerIndex,
                    dispatcher=CasadiStructure.IMDispatcher(struct=self.struct),
                )
        ...
        class FlatIndexGetter(StructureGetter):
            ...
            def __getitem__(self, powerIndex):
                return vec(
                    self.struct.traverseByPowerIndex(
                        powerIndex,
                        dispatcher=CasadiStructure.FlatIndexDispatcher(
                            struct=self.struct
                        ),
                    )
                )
    ...
    def __str__(self, compact=False):
        return (
            "" if compact else "Structure with total size %d.\n" % self.size
        ) + Structure.__str__(self, compact=compact)
    ...
    """
        if i < 0 or i >= self.size:
            raise Exception(
                "Lookup index out of range. Got %d, but structure is of size %d"
                % (i, self.size)
            )
    ...
    def canonicalIndices(self, extraMode=1):
        return [
            self.getCanonicalIndex(i, extraMode=extraMode) for i in range(self.size)
        ]
...
class CasadiStructured(Structured, CasadiStructureDerivable):
    ...
    def __init__(self, struct, order=None):
        ...
        if hasattr(struct, "struct"):
            ...
            Structured.__init__(
                self, CasadiStructure(self.entries, order=entrylist.order)
            )
...
class ssymStruct(CasadiStructured, MasterGettable):
    ...
    def __init__(self, struct, order=None):
        ...
        if any(e.expr is not None for e in self.entries):
            raise Exception(
                "struct_symSX does not accept entries with an 'expr' argument, because such an element is not purely symbolic."
            )
...
class VertsplitStructure:
    def buildMap(self, struct=None, parentIndex=(), parent=None):
        ...
        for it, k, sp, e in zip(its, vertsplit(parent, ks), sps, es):
            ...
            self.priority_object_map[parentIndex + it] = (
                k if k.sparsity() == sp else MX(sp, k)
            )  # [IM(sp,range(sp.nnz()))]
...
class msymStruct(CasadiStructured, MasterGettable, VertsplitStructure):
    ...
    def __init__(self, struct, order=None):
        ...
        if any(e.expr is not None for e in self.entries):
            raise Exception(
                "struct_symMX does not accept entries with an 'expr' argument, because such an element is not purely symbolic."
            )
        if any(e.sym is not None for e in self.entries):
            raise Exception(
                "struct_symMX does not accept entries with an 'sym' argument."
            )
...
class MatrixStruct(CasadiStructured, MasterGettable, MasterSettable):
    ...
    def __init__(self, struct, mtype, data=None, order=None):
        ...
        if any(e.expr is None for e in self.entries):
            raise Exception(
                "struct_SX does only accept entries with an 'expr' argument."
            )
        ...
        if self.master.shape[0] != self.size:
            raise Exception(
                "MatrixStruct: dimension error. Expecting %d-by-1, but got %s"
                % (self.size, self.master.dim())
            )
...
class MXVeccatStruct(CasadiStructured, MasterGettable):
    ...
    def __init__(self, arg, order=None):
        ...
        if any(e.expr is None for e in self.entries):
            raise Exception(
                "struct_MX does only accept entries with an 'expr' argument."
            )
    ...
    def __setitem__(self, powerIndex, value):
        ...
        def inject(payload, canonicalIndex, extraIndex=None, entry=None):
            if extraIndex is not None:
                raise Exception(
                    "An MX veccat structure does not accept indexing on MX level for __setitem__."
                )
            if not hasattr(self, "sparsity"):
                raise Exception(
                    "An MX veccat structure __setitem__ accepts only objects that have sparsity."
                )

            if canonicalIndex in self.mapping:
                if self.struct.map[canonicalIndex].sparsity() != payload.sparsity():
                    raise Exception(
                        "Error in powerIndex slicing %s for canonicalIndex %s: Shape mismatch. lhs is %s, rhs is %s."
                        % (
                            str(powerIndex),
                            str(canonicalIndex),
                            self.struct.map[canonicalIndex].sparsity().dim(),
                            payload.sparsity().dim(),
                        )
                    )
        ...
        return self.struct.traverseByPowerIndex(
            powerIndex, dispatcher=inject, payload=value
        )
    ...
    def master(self):
        if any(e is None for e in self.storage):
            missing = [
                ...
                if self.storage[self.mapping[k]] is None
            ]
            raise Exception(
                "Problem in MX vecNZcat structure cat: missing expressions. The following entries are missing: %s"
                % str(missing)
            )

        if self.dirty:
            self.master_cached = vertcat(
                *[casadi.vec(i if i.is_dense() else i.nz[:]) for i in self.storage]
            )
...
class CasadiStructEntry(StructEntry):
    def __init__(self, *args, **kwargs):
        ...
        if len(args) > 1:
            raise Exception(
                "Don't know what to do with unnamed arguments %s" % str(args[1:])
            )
        ...
        for k in kw:
            if k not in kws:
                raise Exception(
                    "Unknown keyword argument '%s'. Please use one of %s."
                    % (k, str(kws))
                )
        ...
        for kc, fk in [
            ...
            (
                "shapestruct",
                ["struct"],
            ),  # You might have a sparse matrix with shapestruct
            ...
            if kc in kwargs:
                for fki in fk:
                    if fki in kwargs:
                        raise Exception(
                            "You supplied keyword argument '%s', but it cannot be combined with keyword argument '%s'."
                            % (kc, fki)
                        )
        ...
        if "repeat" in kwargs:
            self.repeat = (
                kwargs["repeat"]
                if isinstance(kwargs["repeat"], list)
                else [kwargs["repeat"]]
            )

        if not all([is_integer(x) for x in self.repeat]):
            raise Exception(
                "The 'repeat' argument, if present, must be a list of integers, but got %s"
                % str(self.repeat)
            )
        ...
        if "shape" in kwargs:
            ...
            if is_integer(shape):
                ...
                if len(shape) == 0 or len(shape) > 2:
                    raise Exception(
                        "The 'shape' argument, if present, must be an integer, a tuple of 1 or 2 integers, a sparsity pattern."
                    )
                else:
                ...
                raise Exception(
                    "The 'shape' argument, if present, must be an integer, a tuple of 1 or 2 integers, or a sparsity pattern. Got %s "
                    % str(shape)
                )
        else:
        ...
        if "shapestruct" in kwargs:
            ...
            if isinstance(shapestruct, Structured) or isinstance(
                shapestruct, Structure
            ):
                ...
                if (
                    not (
                        all(
                            [
                                isinstance(e, Structured)
                                or isinstance(e, Structure)
                                or is_integer(e)
                                for e in shapestruct
                            ]
                        )
                    )
                    or len(shapestruct) == 0
                    or len(shapestruct) > 2
                ):
                    raise Exception(
                        "The 'shapestruct' argument, if present, must be a structure or a tuple of structures or numbers"
                    )
                self.shapestruct = tuple(
                    [e if is_integer(e) else e.struct for e in shapestruct]
                )
            else:
                raise Exception(
                    "The 'shapestruct' argument, if present, must be a structure or a tuple of at most structures"
                )

            if "shape" not in kwargs:
                self.sparsity = Sparsity.dense(
                    *[e if is_integer(e) else e.size for e in self.shapestruct]
                )
        ...
        if "sym" in kwargs:
            ...
            if isinstance(sym, SX) and sym.is_valid_input():
                ...
                raise Exception(
                    "The 'sym' argument must be a purely symbolic SX or a structured symbolic. Got %s instead."
                    % str(self.sym)
                )
        ...
        if "expr" in kwargs:
            ...
            if p is None:
                ...
                if hasattr(p, "sparsity"):
                    ...
                    raise Exception(
                        "The 'expr' argument must be a matrix expression or nested list of matrix expressions. Got %s instead."
                        % str(p)
                    )
        ...
        if "type" in kwargs:
            ...
            if self.type not in allowedclass:
                raise Exception(
                    "You supplied a type argument '%s' but it is not recognised. Use one of %s"
                    % (str(self.type, str(allowedclass)))
                )
            if self.type == "symm":
                if self.sparsity.size1() != self.sparsity.size2():
                    raise Exception(
                        "You supplied a type 'symm', but matrix is not square. Got "
                        % self.sparsity.dim()
                        + "."
                    )
        ...
        StructEntry.__init__(
            self, self.name, struct=self.struct, dims=self.repeat, data=self.sparsity
        )
...
class EntryList:
    def __init__(self, arg, order=None):
        ...
        if not isinstance(arg, list):
            raise Exception(
                "Expecting list of entries, with possible tuples for grouping, but got %s"
                % str(arg)
            )
        ...
        if order is not None:
            if any(isinstance(e, tuple) for e in self.order):
                raise Exception(
                    "You supplied an order by using tuple syntax on entries %s, but you overwrite it with the 'order' keyword. Use one or the other, not both."
                )
            self.order = order
        ...
        if len(self.names) != len(set(self.names)):
            ...
            for i, e in enumerate(self.names):
                if e in self.names[:i] or e in self.names[i + 1 :]:
                    duplicates.append(e)
            raise Exception(
                "Your list of entries contains duplicates: %s"
                % str(list(set(duplicates)))
            )
