...
A = np.array([[0, 0, 0], [0, 0, 0], [0, m * g * l / (2 * I), 0]])
...
for t in range(N_steps):
    ...
    if (
        abs(E(observation) - Ed) < 0.05
        and np.cos(observation[2]) < -0.9
        and abs(command_speed) < 4
    ):  # balance
        print("linear control")
        action = 1.0 * ulqr(observation)[0]
    else:
        action = 0.5 * (
            u(observation) / 0.15 - 10.0 * observation[0] - 1.0 * observation[1]
        )  # swing up
